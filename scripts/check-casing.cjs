...
function resolveImport(baseDir, spec) {
  ...
  // handle imports that include a .js/.jsx extension in source by stripping it
  const stripped = spec.replace(/\.(js|jsx)$/, '');
  if (stripped !== spec) {
    const absStripped = path.resolve(baseDir, stripped);
    for (const ext of exts) {
      if (fs.existsSync(absStripped + ext)) return absStripped + ext;
    }
    if (fs.existsSync(absStripped) && fs.statSync(absStripped).isFile()) return absStripped;
    for (const ext of exts) {
      if (fs.existsSync(path.join(absStripped, 'index' + ext))) return path.join(absStripped, 'index' + ext);
    }
  }
